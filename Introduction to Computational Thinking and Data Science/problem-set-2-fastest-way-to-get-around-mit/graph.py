...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        return super().__str__() + " ({}, {})".format(self.total_distance, self.outdoor_distance)
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        in the  graph."""
        src = edge.get_source()
        dist = edge.get_destination()
        if not self.has_node(src):
            raise ValueError("Src node is not added.")
        if not self.has_node(dist):
            raise ValueError("Destination node is not added.")
